...
def from_dyads_to_players_aggregate(df):
    ...
    aggregate = preproc.groups_to_lists(aggregate, 'playerShort')
    data = preproc.drop_columns(aggregate,['birthday', 'player', 'photoID', 'Alpha_3', 'refCountry', 'nIAT', 'nExp','index', 'refNum', 'rater1', 'rater2'])
    data['height'] = preproc.replace_nan(data['height'])
    data['weight'] = preproc.replace_nan(data['weight'])
